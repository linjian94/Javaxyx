...
public class CannonManager {
	...
	public void initCannon(){
		...
		if (cannon != null) {
			...
			if (layoutManager != null) {
				layoutManager.initCannon(cannon);
			}
		}
		setShotable(true);
	}
	...
	public static CannonManager getCannonManager(){
		if(cannonManager==null){
			synchronized(CannonManager.class) {
				if(cannonManager==null){
...
//		SoundManager.playSound(SoundManager.SOUND_BGM_CHANGE_CANNON);
	...
	private void rotateCannon(float targetX,float targetY,Cannon cannon){
		if (cannon == null || cannon.getPicMatrix() == null) {
			return;
		}
		try{
			...
			float gun_angle = Tool.getAngle(targetX, targetY, gamingInfo.getScreenWidth()/2, gamingInfo.getScreenHeight());
	...
	private void resetCannonMatrix(Cannon cannon){
		...
		rotateCannon(gamingInfo.getScreenWidth()/2, 0, cannon);
	}
	/**
	 * �����Ƿ������������
